...
{
    class ProdutoDashboardCP : ConectaDb
    {
        public Double ap { get; set; }
        Double r;
        public List<Produto> BuscarProdutoCCP()
        {
            List<Produto> prod = new List<Produto>();
            try
            {
                MySqlConnection conn = new ConectaDb().getConnection();
                String query = "SELECT * FROM produto WHERE id_produto in (SELECT id_produto FROM compra);";
                conn.Open();
